...
export default function AvanzadoPage() {
  const module = tradingModules.find((m) => m.id === "avanzado")
    ...
    <>
      ...
      <main className="container mx-auto px-4 py-12 text-white">
        <div className="rounded-3xl border border-slate-700 bg-slate-950/80 p-6 shadow-2xl shadow-black/20 sm:p-8">
          <p className="text-sm uppercase tracking-[0.32em] text-lime-300">Trading Avanzado</p>
        ...
        <section className="mt-8">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {module.submodules?.map((submodule) => (
              <Link
                key={submodule.id}
                href={`/trading/avanzado/${submodule.id}`}
